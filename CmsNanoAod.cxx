...
MamaCollection::~MamaCollection()
{
   ...
   for (auto& di : m_items)
   {
      printf("Mammma %s dtor for %d\n", m_class_name.c_str(), i++);
...
void MamaCollection::print_class(FILE *fp)
{
   ...
   fprintf(fp, "{\npublic:\n");
   fprintf(fp,
           "   MamaCollection &m_mama;\n"
           "   int             m_seq_idx;\n"
           "\n"
           "   %s(MamaCollection &mc, int idx) : m_mama(mc), m_seq_idx(idx) {}\n"
           "\n",
           m_class_name.c_str()
           );
   ...
   fprintf(fp, "};\n");
   fprintf(fp, "#pragma link C++ class nanoaod::%s-!;", m_class_name.c_str());
}
...
void Event::autogen_nano_classes(const char* file_name)
{
   FILE *fp = fopen(file_name, "w");

   fprintf(fp, "#ifndef __CLING__\n");
   ...
   fprintf(fp, "#ifndef __ROOTCLING__\n\n");
   ...
   fprintf(fp, "  };\n }\n};\n\n");
   fprintf(fp, "SetterUpper setter_upper;\n\n");
...
void Event::UseTree(TTree *tree)
{
   ...
   if ( ! md5_on_file || strcmp(md5.AsString(), md5_on_file->AsString()) != 0)
   {
      ...
      regen = true;
      autogen_nano_classes("CmsNanoClasses.hxx");
   ...
   if (TClass::GetClass("nanoaod::Electron") || TClass::GetClass("nanoaod::Jet"))
   {
      if (regen) {
         printf("Error: nano classes already linked-in but apparently wrong version\n");
         gSystem->Exit(1);
      } else {
         printf("nano classes already linked-in.\n");
      }
   }
   else
   {
      printf("Loading in nano classes.\n");
      gROOT->ProcessLine(".L CmsNanoClasses.hxx");
   }
...
void Event::RegisterMamaCollection(const std::string& coll_name)
{
   ...
   m_mama_colls.push_back( new MamaCollection(coll_name) );
}
...
void CmsNanoAod()
{
   printf("CmsNanoAod apparently loaded through .x\n");
}
